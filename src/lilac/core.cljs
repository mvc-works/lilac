...
(defn validate-vector [data rule coord]
  (let [item-rule (:item rule), coord (conj coord 'vector), allow-list? (:allow-list? rule)]
    (if (or (vector? data) (if allow-list? (list? data) false))
      (loop [xs data, idx 0]
        (if (empty? xs)
          {:ok? true}
          (let [x0 (first xs)
...
(defn validate-list [data rule coord]
  (let [item-rule (:item rule), coord (conj coord 'list), allow-vector? (:allow-vector? rule)]
    (if (or (list? data) (if allow-vector? (vector? data) false))
      (loop [xs data, idx 0]
        (if (empty? xs)
          {:ok? true}
          (let [x0 (first xs)
...
(defn list+
  ...
  ([item options]
   {:lilac-type :list,
    :item item,
    :options options,
    :allow-vector? (:allow-vector? options)}))
...
(defn vector+
  ...
  ([item options]
   {:lilac-type :vector, :item item, :options options, :allow-list? (:allow-list? options)}))
